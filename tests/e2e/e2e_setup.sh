...
aws kms put-key-policy --policy-name default --key-id "${KMS_KEY_ARN}" --policy file://key_policy.json
aws lambda invoke --function-name "${FUNCTION_ARN}" --cli-binary-format raw-in-base64-out --payload '{"operation": "set_tls_key"}' lambda-output
